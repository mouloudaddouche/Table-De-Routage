		  ...
		  // String edgeStyle = mxConstants.STYLE_EDGE + "=" + mxConstants.EDGESTYLE_TOPTOBOTTOM;		  
		   for(int i=0;i<20;i++) {
			   for (Lien l : G.tab[i].getConnexion()) {
				   if(G.tab[i].getType()==l.getN().getType()) { 
					  tabedge[k]= graph.insertEdge(parent, null,l.getValeur() , tabnode[G.tab[i].getId()-1], tabnode[l.getN().getId()-1],"dashed=1;fontColor=#FF0000;");k++;}
				   else { tabedge[k]=graph.insertEdge(parent, null,l.getValeur() , tabnode[G.tab[i].getId()-1], tabnode[l.getN().getId()-1],"fontColor=#000000;");k++;}
			   }
		   }
		}
		finally
			...
			public void mouseReleased(MouseEvent e){
					...
					if (cell != null)
					{
						...
						case 1:{
							...
							i++;
							Iterator <Noeud> iter= G.tab[noeudsrc-1].getTable_Hashage().get(noeuddest-1).iterator();
							while(iter.hasNext()){
								graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, "red", new Object[]{tabnode[iter.next().getId()-1]});
							}
