...
public void Creation_Aleatoire() {
	for (int i=0;i<20;i++){
		if (i<4) { // Blackbone
		...
		else if (i>=4 && i<10){ //Tier2
			//Lien avec Blackbone
			int indice;
			while(tab[i].getNbtier1()>0){
				do {
				indice=Outils.Rand(3,0);}while(tab[i].Lien_Existe(tab[indice]));
			...
			if(tab[i].getNbtier3()>0) {
				do {
		    indice=Outils.Rand(19,10);} while((tab[indice].getNbtier3()==0)||(tab[i].Lien_Existe(tab[indice]) ||(i==indice)));
			int valeur=Outils.Rand(15,50);
			tab[i].Connexion.add(new Lien(tab[indice],valeur));
			tab[indice].Connexion.add(new Lien(tab[i],valeur));
	...
	for (int i=0;i<20;i++){
		Collections.sort(this.tab[i].Connexion);
	}
